...
#include <iostream>
const int max_buffer_size = 300; //In {150,200,250,300}, 300 is the best
namespace sjtu {
    template<class T>
    class list {
        struct node {
    ...
    public:
        ...
        public:
            ...
            iterator operator -(size_t u) const{
                // if(*this == rt->getend()){   //PREVIOUS_VERSION
                //     return u ? rt->gettail() - (u-1) : rt->getend();
                // }
                if(!pos){ //注意链表的end指向的是空指针!要转换成tail
                    return u ? rt->gettail() - (u-1) : rt->getend();
                }
                node* tmp = pos;
        ...
        // list(iterator _head, iterator _tail, const size_t _size){
        //     head = _head.getpos();
        //     tail = _tail.getpos();
        //     size = _size;
        // }
...
public:
	...
	iterator insert(iterator pos, const T &value) {
        ...
        if(pos.chain()->getsize() >= 2 * max_buffer_size){
            list<T> * tmp = pos.chain()->split(max_buffer_size);
	...
	iterator erase(iterator pos){
        ...
        if(pos.chain()->empty()){ //删完之后若最后一个链表块为空
            size_t shift = pos.chain() - List.getbegin(); //shift为两个iterator的index之差
            typename list<list<T> >::iterator tmp = List.erase(pos.chain()); //将空链表块删去
            if(tmp == List.getend()) ret = end(); //if pos pointing to the last element, end() will be returned.
            else{
                tmp = List.getbegin() + shift; //主迭代器移动shift个链表块
                ret = iterator(tmp, tmp->getbegin()); //指向下一个链表块的第一个结点
            }
        }
        else if(ret.sub() == ret.chain()->getend() && ret != end()){ 
            //如果该链表块为空且为非末端链表块,指向下一个链表块第一个结点
